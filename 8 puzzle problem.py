...
def solve_8_puzzle(start, goal):
    # Pre-calculate goal positions for O(1) lookups in heuristic
    goal_positions = {val: (i, j) for i, row in enumerate(goal)
                      for j, val in enumerate(row) if val != 0}

    open_list = []
    initial_f = heuristic(start, goal_positions)
    heappush(open_list, (initial_f, start, []))
    closed_list = set()
...
def heuristic(state, goal_positions):
    return sum(abs(i - goal_positions[val][0]) +
               abs(j - goal_positions[val][1])
