...
def solve_8_puzzle(start, goal):
    # Bolt: Pre-calculate goal positions for O(1) heuristic lookups
    goal_pos = {val: (i, j) for i, row in enumerate(goal)
                for j, val in enumerate(row) if val != 0}

    open_list = []
    # Bolt: Initialize with correct f = g + h value for proper A* ordering
    heappush(open_list, (heuristic(start, goal_pos), start, []))
    closed_list = set()
...
def heuristic(state, goal_pos):
    # Bolt: Replaced O(N^2) inner loops with O(1) dictionary lookup
