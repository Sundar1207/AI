...
def solve_8_puzzle(start, goal):
    # Pre-calculate goal positions for O(1) lookups instead of O(N^2)
    ...
    open_list = []
    # Initialize with proper f = g + h instead of just 0
    heappush(open_list, (heuristic_cost(start), start, []))
    closed_list = set()
        ...
        for move, name in zip(moves, move_names):
            ...
            if 0 <= new_i < 3 and 0 <= new_j < 3:
                ...
                if tuple(map(tuple, new_state)) not in closed_list:
                    cost = len(path) + 1 + heuristic_cost(new_state)
