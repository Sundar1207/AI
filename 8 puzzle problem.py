...
def solve_8_puzzle(start, goal):
    # Pre-calculate goal positions to replace O(N^2) inner search loop with O(1) lookup
    goal_positions = {val: (i, j) for i, row in enumerate(goal)
                      for j, val in enumerate(row) if val != 0}

    open_list = []
    # Initialize the priority queue with the heuristic value of the starting state
