...
def solve_8_puzzle(start, goal):
    # ⚡ Bolt Optimization: Pre-compute goal positions for O(1)
    # lookups in heuristic
        ...
        for move, name in zip(moves, move_names):
            ...
            if 0 <= new_i < 3 and 0 <= new_j < 3:
                ...
                if tuple(map(tuple, new_state)) not in closed_list:
                    cost = len(path) + 1 + heuristic(new_state, goal, goal_pos)
                    heappush(open_list, (cost, new_state, path + [name]))
    return None


def heuristic(state, goal, goal_pos=None):
    # Fallback if goal_pos isn't provided (maintains backwards compatibility)
    if goal_pos is None:
        goal_pos = {val: (gi, gj) for gi, grow in enumerate(goal)
