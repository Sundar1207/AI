...
def solve_8_puzzle(start, goal):
    # Optimize: Pre-calculate goal positions for O(1) heuristic lookups
    goal_positions = {val: (i, j) for i, row in enumerate(goal)
                      for j, val in enumerate(row) if val != 0}
    open_list = []
    # Initialize with f = g + h, where g=0 and h=heuristic(start)
    heappush(open_list, (heuristic(start, goal_positions), start, []))
    closed_list = set()
