...
def solve_8_puzzle(start, goal):
    # Pre-calculate goal positions for O(1) lookup
    goal_pos = {val: (i, j) for i, row in enumerate(goal)
                for j, val in enumerate(row) if val != 0}

    # Initialize heuristic f = g + h
    initial_h = sum(abs(i - goal_pos[val][0]) + abs(j - goal_pos[val][1])
        ...
        for move, name in zip(moves, move_names):
            ...
            if 0 <= new_i < 3 and 0 <= new_j < 3:
                ...
                if tuple(map(tuple, new_state)) not in closed_list:
                    h = sum(abs(i - goal_pos[val][0]) + abs(j - goal_pos[val][1])
